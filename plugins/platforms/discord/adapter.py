...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def _reply_reference_for_send(self, reply_to, channel):
        ...
        if not reply_to or self._reply_to_mode == "off" or str(reply_to) in self._voice_message_reply_suppression_ids:
            return None
        try:
    ...
    def _attachment_message_type(self, att: Any, *, captionless: bool = False) -> MessageType:
        ...
        if content_type.startswith("audio/"):
            ...
            return MessageType.VOICE if self._is_discord_voice_message_attachment(att) or (captionless and filename.endswith((".ogg", ".opus"))) else MessageType.AUDIO
    ...
    async def _handle_message(
        ...
        if normalized_content.startswith("/"):
            ...
            msg_type = self._attachment_message_type(all_attachments[0], captionless=not normalized_content.strip())
        else:
