...
def _is_ibkr_public_mcp(server_name: str | None = None, server_url: str | None = None) -> bool:
    """True only for IBKR's hosted public MCP endpoint."""
    from utils import base_url_host_matches, base_url_hostname
    url = (server_url or "").lower()
    if base_url_host_matches(url, "api.ibkr.com") and "/mcp" in url:
        return True
    return "ibkr" in (server_name or "").lower() and (not url or "ibkr" in base_url_hostname(url))


def prepare_oauth_flow_request(request, *, server_url: str | None, user_agent: str | None, context=None):
    """Apply IBKR compatibility to SDK OAuth discovery and registration requests."""
    url = str(request.url)
    path = urlparse(url).path or ""
    if user_agent and ("/.well-known/" in path or path.endswith("/register")):
    ...
    if context is not None and _is_ibkr_public_mcp(server_url=server_url):
        if path.endswith("/register"):
            ...
            headers = {key: value for key, value in request.headers.items() if key.lower() not in {"content-length", "content-type", "host"}}
            headers["Content-Type"] = "application/json"
            request = type(request)(request.method, url, headers=headers, content=json.dumps(payload, separators=(",", ":")).encode("utf-8"))
        if url.startswith("https://api.ibkr.com/oauth2/.well-known/") or "/.well-known/oauth-authorization-server/oauth2" in url:
...
def apply_oauth_provider_defaults(cfg: dict, *, server_name: str = "", server_url: str | None = None) -> dict:
    ...
    if _is_ibkr_public_mcp(server_name, server_url):
        cfg["scope"] = cfg.get("scope") or _IBKR_DEFAULT_SCOPE
        cfg["user_agent"] = cfg.get("user_agent") or _IBKR_DEFAULT_USER_AGENT
    return cfg
